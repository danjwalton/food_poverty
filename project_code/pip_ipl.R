...
#Read in extreme poverty lines
ipls <- fread("ipl_2023.csv")

#Remove countries with no ipl for 2017
ipls <- ipls[!is.na(ipl_2017)]
...
if(length(list.files(pattern = "^extreme_poverty_2023[.]csv$")) == 1){
  ipl_out <- fread("extreme_poverty_2023.csv")
} else {
...
for(i in 1:nrow(ipls_todo)){
  ...
  fwrite(ipl_out, "extreme_poverty_2023.csv")
}
...
fwrite(extreme_poor_total, "extreme_poor_total.csv")
...
fwrite(extreme_poor_cc, "extreme_poor_cc.csv")
